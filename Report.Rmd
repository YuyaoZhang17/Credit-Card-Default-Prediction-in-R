...
pROC: This is the package we use to visualize, smooth and compare receiver operating characteristic and plot the ROC. I t can also be used to calculate the AUC which we used to compare the results of different models' performance.
...
The result of GBM is not better than random forest for two reasons: First, our dataset contains 24 features to predict the default outcome, this structure requires the width of model to train but not that much on the depth of training. So that the deepening training method GBM fails to capture as many features as random forest; Second, random forest has the advantage of parallel computing which helps to train high dimension dataset but GBM links consequent trees orderly and can not be used to train parallelly. 
